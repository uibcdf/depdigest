...
SPEC = importlib.util.spec_from_file_location("depdigest_release_route", SCRIPT)
...
def test_required_ci_must_have_exact_identity_and_success(monkeypatch, wrong_field):
    ...
    with pytest.raises(route.ReleaseRouteError, match="no successful exact-commit run"):
        route.verified_workflow_runs(
            "uibcdf/depdigest", SHA, [route.FULL_MATRIX], "token"
        )
...
def test_direct_preflight_accepts_only_explicit_404(monkeypatch):
    ...
    assert route.assert_version_unoccupied("0.11.0")["state"] == "absent"


@pytest.mark.parametrize("distributions", [[{"basename": "noarch/depdigest-py_0"}], []])
def test_direct_preflight_rejects_any_existing_version(monkeypatch, distributions):
    ...
    with pytest.raises(route.ReleaseRouteError, match="direct upload is forbidden"):
        route.assert_version_unoccupied("0.11.0")
...
def test_public_poststate_matches_the_exact_built_file(monkeypatch, tmp_path):
    receipt = tmp_path / "receipt.json"
    receipt.write_text(
        json.dumps({"version": "0.11.0", "route": "direct"}), encoding="utf-8"
    )
    package = tmp_path / "depdigest-0.11.0-py_0.tar.bz2"
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args=[], returncode=0, stdout=json.dumps({"depdigest": [record]}), stderr=""
        ),
    )
    evidence = route.verify_public(
        version="0.11.0", built_paths=str(package), receipt=receipt, attempts=1
    )
