...
def test_stable_releases_have_a_guarded_direct_route():
    ...
    assert (
        "depdigest-conda-${{ inputs.version || github.event.release.tag_name }}"
        in workflow
    )
...
def test_noarch_workflow_has_one_job_and_retains_producer_evidence():
    ...
    assert (
        "depdigest-conda-route-${{ github.run_id }}-${{ github.run_attempt }}"
        in workflow
    )
...
def test_promotion_workflow_checks_exact_release_and_file_identity():
    ...
    assert (
        "uibcdf/depdigest/$RELEASE_VERSION/noarch/depdigest-$RELEASE_VERSION-py_$BUILD_NUMBER.tar.bz2"
        in workflow
    )
