"""Checking an exact DepDigest Conda release route and its public outcome."""
...
def assert_version_unoccupied(version: str) -> dict:
    ...
    url = f"https://api.anaconda.org/release/uibcdf/depdigest/{quote(version, safe='')}"
    try:
    ...
    raise ReleaseRouteError(
        f"DepDigest {version} already has {len(distributions)} distribution records; "
...
def check_route(
    *, version: str, route: str, sha: str, repository: str, receipt: Path
    ...
    if repository != "uibcdf/depdigest":
        raise ReleaseRouteError("release route must run in uibcdf/depdigest")
    plan = read_plan()
...
def verify_public(
    *, version: str, built_paths: str, receipt: Path, attempts: int = 6
    ...
    expected_name = f"depdigest-{version}-py_0.tar.bz2"
    ...
    for attempt in range(attempts):
        ...
        if result.returncode == 0:
            records = json.loads(result.stdout).get("depdigest", [])
            matching = [
                record
            ...
            if len(matching) == 1:
                evidence["public"] = {
                    "coordinate": f"uibcdf/depdigest/{version}/noarch/{expected_name}",
                    "sha256": digest,
