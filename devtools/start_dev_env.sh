...
# ------------------------------------------------------------------------------
# DepDigest development bootstrap with activation switch & flexible path discovery
# - Creates/updates an environment from development_env.yaml
# - Installs DepDigest in editable mode (no deps)
...
#     * parent-of-repo, repo root, depdigest/, or devtools/
#
# Usage examples:
...
print_help() {
  cat <<'EOF'
Usage: devtools/start_dev_env.sh [OPTIONS]
...
Options:
  ...
  --env-name <name>
      Environment name (default: depdigest)

  --env-yaml <path>
      Path to the YAML file. If omitted, the script auto-discovers it
      (works when called from parent-of-repo, repo root, depdigest/, or devtools/).
...
CWD_CANDIDATES=(
  "depdigest/devtools/conda-envs/development_env.yaml"
...
SCRIPT_CANDIDATES=(
  "${REPO_ROOT}/depdigest/devtools/conda-envs/development_env.yaml"
...
# --- Editable install -----------------------------------------------------------
echo "[setup] Installing DepDigest in editable mode (no deps)..."
${RUN} python -m pip install --no-deps --editable .

# --- Optional kernel registration ----------------------------------------------
if [[ "${REGISTER_KERNEL}" == "1" ]]; then
  echo "[setup] Registering Jupyter kernel 'Python (depdigest)'..."
  ${RUN} python -m ipykernel install --user --name "${ENV_NAME}" --display-name "Python (depdigest)"
fi
